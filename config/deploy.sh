...
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i $pem_file $remote_server "docker run --name hostplat-server -p 9000:9000 ndakic/hostplat-server:latest </dev/null >/dev/null 2>&1 &"
